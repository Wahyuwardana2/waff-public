...
function StoreItem() {
  ...
  if (status === "loading") {
    ...
    return (
      <>
        <Helmet>
          <title>{product.name} - {store.storeName} - W-Aff</title>
        </Helmet>
        <div className="poppins">
          <button
            className={`md:hidden flex gap-1 items-center font-medium cursor-pointer mb-3  ${store.colorTheme + "Nav"
              } `}
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 my-6 gap-2 md:gap-6">
            ...
            <div className="col-span-1 p-5 flex flex-col gap-2">
              <button
                className={`hidden md:flex gap-1 items-center font-medium cursor-pointer mb-3  ${store.colorTheme + "Nav"
                  } `}
                onClick={() => navigate(`/${store.storeName}`)}
              >
                <Icon icon="charm:chevron-left" width={20} />
              ...
              <div className="w-full flex flex-col gap-[10px] mt-4">
                ...
                <button
                  className={`p-[10px] rounded-full font-semibold text-lg text-white ${store.colorTheme + "-btn"
                    }
