    ...
    <>
        ...
        <tbody {...getTableBodyProps()}>
          {page.map((row, i) => {
            ...
            return (
              <tr
                {...row.getRowProps()}
                className="border-y-[1px] text-gray-800 border-gray-300 cursor-pointer hover:bg-orange-100  helveticaFonts"
      ...
      <div className="flex gap-3 items-center justify-end mt-6 mb-2 mx-2">
        ...
        <p className="font-medium text-gray-600">
          <span className="text-orange-700">{pageIndex + 1}</span> dari{" "}
          {pageOptions.length}
        </p>
        <button disabled={!canNextPage} onClick={() => nextPage()}>
          <Icon
            ...
            width="28"
            className={`${!canNextPage ? "opacity-40 cursor-auto" : "hover:text-orange-600 text-gray-700"
              } cursor-pointer`}
          />
        </button>
      </div>
